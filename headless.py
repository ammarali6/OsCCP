...
def run_headless_simulation(
    ...
    memory_waits = [
        {"pid": p["pid"], "memory_wait": p.get("memory_wait", 0)} for p in result
    ]
    avg_memory_wait = (
        round(sum(p.get("memory_wait", 0) for p in result) / len(result), 2)
        if result
        else 0
    )
    ...
    try:
        ...
        viz.plot_partitions(
            mem.get_block_state(), total_size=memory_total, save_path=mem_file
        )
        plt.close("all")
    except Exception as e:
    ...
    with open(details_file, "w") as f:
        ...
        for p in result:
            f.write(
                f"  {p['pid']}: req={p.get('memory_req', 0)}, "
...
def run_with_memory_constraints(algorithm, processes, memory_manager, quantum=2):
    ...
    if newly_ready:
        ...
        if algorithm == "fcfs":
            ...
            nr_result, nr_gantt = round_robin.run_round_robin(
                newly_ready, quantum=quantum
            )
        elif algorithm == "mlfq":
